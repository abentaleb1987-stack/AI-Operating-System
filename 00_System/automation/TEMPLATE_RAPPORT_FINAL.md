# Rapport final AOS - {{Date}} - {{Sujet}}

## Synthese batch

- Sources detectees : {{Nombre sources detectees}}
- Sources traitees : {{Nombre sources traitees}}
- Sources echouees : {{Nombre sources echouees}}
- IA principales detectees : {{Liste IA principales}}
- Decision finale batch : {{GO / GO partiel / A surveiller / Rejete / Blocage critique}}

## Sources traitees

- {{Source traitee}}

## Nommage des sources traitees

- Nom source initial : {{Nom source initial}}
- Nom source traite final : {{Nom source traite final}}
- Chemin final dans traitees : {{Chemin final traitees}}

## Sources echouees

- {{Source echouee}} - {{Cause}} - {{Action}}

## Detail par source

- Source : {{Chemin source}}
- IA principale : {{IA principale}}
- Outils / frameworks : {{Outils ou frameworks}}
- Fiche de veille creee : {{Fiche de veille}}
- Fiche permanente modifiee : {{Fiche permanente ou Non}}
- Sections modifiees : {{Sections}}
- Source deplacee : {{Oui / Non}} - {{Chemin destination}}
- Nom source initial : {{Nom source initial}}
- Nom source traite final : {{Nom source traite final}}
- Chemin final dans traitees : {{Chemin final traitees}}
- Decision source : {{GO / A surveiller / Rejete / Echec}}

## IA / outils detectes

- IA principale : {{IA principale}}
- IA secondaires : {{IA secondaires}}
- Outils / frameworks : {{Outils ou frameworks}}

## Fichiers crees

- {{Fichier cree}}

## Fichiers modifies

- {{Fichier modifie}}

## Fiches permanentes mises a jour

- {{Fiche permanente}}

## Sections modifiees

- {{Section modifiee}}

## Connaissances integrees

- {{Connaissance integree}}

## Connaissances non integrees

- {{Connaissance non integree}} - {{Raison}}

## Points a surveiller

- {{Point a surveiller}}

## Source deplacee vers traitees

- {{Chemin source traitee}}

## Sources deplacees

- {{Nom source initial}} -> {{Nom source traite final}} -> {{Chemin traitees}}

## Git

- Commit : {{Hash commit}}
- Push : {{Resultat push}}
- Statut final : {{Resultat git status --short}}

## Decision finale

{{GO / GO partiel / A surveiller / Rejete}}
